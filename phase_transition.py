...
for dir in tqdm(dirs):
    ...
    for s in ss:
        ...
        if len(sizes) == 0:
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_component_hist_e_iter{s}"] = np.nan
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_component_hist_p_iter{s}"] = np.nan
        else:
            p, e = hist(sizes, bins=50)
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_component_hist_e_iter{s}"] = e
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_component_hist_p_iter{s}"] = p
        max_cluster_sizes.append(np.max(sizes))
        clique_sizes = np.array(data['clique_sizes_cth0.5'])
        if len(clique_sizes) == 0:
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_hist_e_iter{s}"] = np.nan
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_hist_p_iter{s}"] = np.nan
        else:
            p, e = hist(clique_sizes, bins=50)
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_hist_e_iter{s}"] = e
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_hist_p_iter{s}"] = p

        values = np.array(data['clique_values_cth0.5'])
        if len(values) == 0:
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_value_hist_e_iter{s}"] = np.nan
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_value_hist_p_iter{s}"] = np.nan
        else:
            try:
                p, e = hist(values, bins=50)
            except ValueError:
                p, e = np.nan, np.nan
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_value_hist_e_iter{s}"] = e
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_clique_value_hist_p_iter{s}"] = p
        
        degrees = np.array(data['degree_cth0.5'])
        if len(degrees) == 0:
            average_degrees.append(0)
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_degree_hist_e_iter{s}"] = np.nan
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_degree_hist_p_iter{s}"] = np.nan
        else:
            average_degrees.append(np.mean(degrees))
            p, e = hist(degrees, bins=50)
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_degree_hist_e_iter{s}"] = e
            data_dict[f"init{init}_lr{lr}_bs{bs}_neurons{neurons}_degree_hist_p_iter{s}"] = p
